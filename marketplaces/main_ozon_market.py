...
from api_ozon.finance_transaction import Ozon
from utils.general import GeneralUtils
from greenplum.sqlalchemy_manager import GreenPlum

# Set up argument parsing
def parse_arguments():
    parser = argparse.ArgumentParser(description="Run Ozon Market report script with custom date and timedelta.")
...
def main(report_date_to, report_date_from):
    ...
    port = "5432"
    source_data_table_name = 'ozon_finance_transactions_sources'
    transformed_data_table_name = 'ozon_finance_transactions'
    logger_name = f"{report_date_from.strftime('%Y-%m-%d')}_Ozon_market"
    ...
    oz = Ozon(ozon_api_key=os.getenv('OZON_API_KEY'), 
    ...
    result = oz.get_all_finance_transactions()
    ...
    df = pd.DataFrame()
    df = oz.simplify_response(result)

    # Data quality check
    dqc.check_missing_values(df)
    dqc.check_data_positive(df, ['operation_id', 'items_sku'])
    dqc.check_for_duplicates(df, ['operation_id'])
...
if __name__ == "__main__":
    ...
    while report_date_to >= report_date_from:
        main(report_date_from, report_date_from)
